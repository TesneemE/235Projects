...
//        */
bool Character::operator==(const Character &x) const
{
    string x_race=x.getRace();
    int x_race_int;
    string race_array[6]={"NONE","HUMAN","ELF","DWARF","LIZARD","UNDEAD"};
    for(int i=0;i<6;i++)
    {
        if (x_race==race_array[i])
        {
            x_race_int=i;
        }
    }
    int char_race=race_;
    if( (name_==x.getName()) && (char_race==x_race_int) && (level_==x.getLevel()) && (enemy_==x.isEnemy()) )
    {
        return true;
    }
    else
...
{
    cout<<name_<<" is a Level "<<level_<<" "<<race_<<".\n"<<"Vitality: "<<vitality_<<"\n"<<"Max Armor: "<<armor_<<"\n";
    if(enemy_==true)
    {
